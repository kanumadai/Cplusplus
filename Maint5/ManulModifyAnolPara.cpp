...
BOOL CManulModifyAnolPara::OnInitDialog() 
{
	...
	memset(m_arrAnolParaPreData,0,32*8*10);
	GetDlgItem(IDC_MANUAL_MOD)->ShowWindow(false);  
	GetDlgItem(IDC_MANUAL_MOD_PRE)->ShowWindow(true); 
...
void CManulModifyAnolPara::OnAbcdModify() 
{
	...
	for(int bk=0; bk<m_iStatCounter; bk++) {
		for(int block=0;block<8;block++) {
			...
			m_Grid.SetCol(1);
			m_Grid.SetCellBackColor(RGB(232,87,38));
			if( PeakPositionPara[8*bk+block] == ipeakEth) { //颜色为绿色
				m_Grid.SetCellBackColor(RGB(30,189,169));  
			}
...
//调整偏置参数X,Y,E 
//读区第一行参数的X,Y,E 作为修改数据！！ 改变所有的bucket 的x,y,e;
void CManulModifyAnolPara::OnOffsetModify() 
{
	CString strX,strY,strE;

	if (m_iStatCounter <= 0) return;
...
void CManulModifyAnolPara::OnManualModPre() 
{
	for(int bk=0; bk<m_iStatCounter; bk++) {
		for(int block=0;block<8;block++) {
			...
			m_Grid.SetCol(1);			
			m_arrAnolParaPreData[bk][block][0] = atoi(m_Grid.GetText());
			m_Grid.SetCellBackColor(RGB(30,189,169));
			m_Grid.SetText("0");

			m_Grid.SetCol(2);
			m_arrAnolParaPreData[bk][block][1] = atoi(m_Grid.GetText());
			m_Grid.SetCellBackColor(RGB(30,189,169));
			m_Grid.SetText("0");

			m_Grid.SetCol(3);
			m_arrAnolParaPreData[bk][block][2] = atoi(m_Grid.GetText());
			m_Grid.SetCellBackColor(RGB(30,189,169));
			m_Grid.SetText("0");

			m_Grid.SetCol(4);
			m_arrAnolParaPreData[bk][block][3] = atoi(m_Grid.GetText());
			m_Grid.SetCellBackColor(RGB(30,189,169));
			m_Grid.SetText("0");
			
			m_Grid.SetCol(5);
			m_arrAnolParaPreData[bk][block][4] = atoi(m_Grid.GetText());
			m_Grid.SetCellBackColor(RGB(30,189,169));
			m_Grid.SetText("0");

			m_Grid.SetCol(6);
			m_arrAnolParaPreData[bk][block][5] = atoi(m_Grid.GetText());
			m_Grid.SetCellBackColor(RGB(30,189,169));
			m_Grid.SetText("0");

			m_Grid.SetCol(7);
			m_arrAnolParaPreData[bk][block][6] = atoi(m_Grid.GetText());
			m_Grid.SetCellBackColor(RGB(30,189,169));
			m_Grid.SetText("0");

			m_Grid.SetCol(8);
			m_arrAnolParaPreData[bk][block][7] = atoi(m_Grid.GetText());
			m_Grid.SetCellBackColor(RGB(30,189,169));
			m_Grid.SetText("0");

			m_Grid.SetCol(9);
			m_arrAnolParaPreData[bk][block][8] = atoi(m_Grid.GetText());

		}
	}
	GetDlgItem(IDC_MANUAL_MOD)->ShowWindow(true);  
	GetDlgItem(IDC_MANUAL_MOD_PRE)->ShowWindow(false);  
...
void CManulModifyAnolPara::OnManualMod() 
{
	...
	for(int bk=0; bk<m_iStatCounter; bk++) {
		for(int block=0;block<8;block++) {
			...
			m_Grid.SetCol(1);			
			iAnolPara = atoi(m_Grid.GetText());			
			strtext.Format("%d",m_arrAnolParaPreData[bk][block][0] + iAnolPara);			
			m_Grid.SetCellBackColor(RGB(30,189,169));
			if(iAnolPara != 0) m_Grid.SetCellBackColor(RGB(232,87,38));
			m_Grid.SetText(strtext);

			m_Grid.SetCol(2);
			iAnolPara = atoi(m_Grid.GetText());
			strtext.Format("%d",m_arrAnolParaPreData[bk][block][1] + iAnolPara);
			m_Grid.SetCellBackColor(RGB(30,189,169));
			if(iAnolPara != 0) m_Grid.SetCellBackColor(RGB(232,87,38));
			m_Grid.SetText(strtext);
			...
			m_Grid.SetCol(3);
			iAnolPara = atoi(m_Grid.GetText());
			strtext.Format("%d",m_arrAnolParaPreData[bk][block][2] + iAnolPara);
			m_Grid.SetCellBackColor(RGB(30,189,169));
			if(iAnolPara != 0) m_Grid.SetCellBackColor(RGB(232,87,38));
			m_Grid.SetText(strtext);
			...
			m_Grid.SetCol(4);
			iAnolPara = atoi(m_Grid.GetText());
			strtext.Format("%d",m_arrAnolParaPreData[bk][block][3] + iAnolPara);
			m_Grid.SetCellBackColor(RGB(30,189,169));
			if(iAnolPara != 0) m_Grid.SetCellBackColor(RGB(232,87,38));
			m_Grid.SetText(strtext);
			...
			m_Grid.SetCol(5);
			iAnolPara = atoi(m_Grid.GetText());
			strtext.Format("%d",m_arrAnolParaPreData[bk][block][4] + iAnolPara);
			m_Grid.SetCellBackColor(RGB(30,189,169));
			if(iAnolPara != 0) m_Grid.SetCellBackColor(RGB(232,87,38));
			m_Grid.SetText(strtext);
			...
			m_Grid.SetCol(6);
			iAnolPara = atoi(m_Grid.GetText());
			strtext.Format("%d",m_arrAnolParaPreData[bk][block][5] + iAnolPara);
			m_Grid.SetCellBackColor(RGB(30,189,169));
			if(iAnolPara != 0) m_Grid.SetCellBackColor(RGB(232,87,38));
			m_Grid.SetText(strtext);
			...
			m_Grid.SetCol(7);
			iAnolPara = atoi(m_Grid.GetText());
			strtext.Format("%d",m_arrAnolParaPreData[bk][block][6] + iAnolPara);
			m_Grid.SetCellBackColor(RGB(30,189,169));
			if(iAnolPara != 0) m_Grid.SetCellBackColor(RGB(232,87,38));
			m_Grid.SetText(strtext);
			...
			m_Grid.SetCol(8);
			iAnolPara = atoi(m_Grid.GetText());
			strtext.Format("%d",m_arrAnolParaPreData[bk][block][7] + iAnolPara);
			m_Grid.SetCellBackColor(RGB(30,189,169));
			if(iAnolPara != 0) m_Grid.SetCellBackColor(RGB(232,87,38));
			m_Grid.SetText(strtext);

			//m_Grid.SetCol(9);
			//m_iarrAnolParaPreData[bk][block][7] = atoi(m_Grid.GetText());
